        ...
        <form @submit.prevent="submit" class="bg-white rounded-lg shadow-md p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <!-- Foto actual -->
                    <div v-if="maquina.foto && !previewUrl" class="mb-4">
                        <p class="text-sm text-gray-600 mb-2">Foto actual:</p>
                        <img :src="`/storage/${maquina.foto}`" alt="Foto actual" class="w-32 h-32 object-cover rounded border">
                    </div>
...
const form = useForm({
    codigo: props.maquina.codigo,
    nombre: props.maquina.nombre,
    tipo_maquina_id: props.maquina.tipo_maquina_id,
    fecha_compra: props.maquina.fecha_compra,
    horas_trabajadas: props.maquina.horas_trabajadas,
    estado: props.maquina.estado,
    ubicacion_planta: props.maquina.ubicacion_planta,
    foto: null,
    _method: 'PUT',
});
...
const submit = () => {
    form.post(`/maquinas/${props.maquina.id}`);
};
</script>
